...
(function (videojs) {
    ...
    constructor: function constructor(player, options) {
      ...
      this.on("statechanged", function (e) {
        ...
        if (e.changes && e.changes.customClass) {
          this.player.removeClass(e.changes.customClass.from).addClass(e.changes.customClass.to);
        }
      });
      ;
  ...
  DanmuPlugin.prototype.logState = function () {// videojs.log(`the player is now ${this.state.playing ? 'playing' : 'paused'}`);
  };

  videojs.registerPlugin('danmuPlugin', DanmuPlugin); // console.log('注册')
})(videojs); // danmuPlugin.defaultState = {
...
var DanButton = videojs.extend(Button, {
  constructor: function constructor(player, options) {
    ...
    this._options_ = options; // this.controlText("弹幕开关");
    // this.controlText(options && options.controlText || this.localize('Barrage Switch'));

    this.isShowDanmu = true; // this.on("click", this.handleClick);
  ...
  handleClick: function handleClick(e) {
    ...
    if (this.isShowDanmu) {
      ...
      danmuBtn.className = className.replace(/show/g, 'hide'); //  danmuBtn.removeClass("show").addClass("hide")

      this._options_ && this._options_.hideDanmu && this._options_.hideDanmu(); // console.log("关闭弹幕");
    } else {
      this.isShowDanmu = true;
      danmuBtn.className = className.replace(/hide/g, 'show'); // danmuBtn.removeClass("hide").addClass("show")

      this._options_ && this._options_.showDanmu && this._options_.showDanmu(); // console.log("显示弹幕");
    }
  }
});
videojs.registerComponent('DanmuButton', DanButton); // 弹幕容器
// var Commponet = videojs.getComponent('Component');

var DanView = videojs.extend(Commponet, {
  constructor: function constructor(player, options) {
    this.player_ = player;
    Commponet.apply(this, arguments); // this.el()
